...
| `species` | VARCHAR | Không | | Loại: `dog`, `cat`, `bird`, `fish`, `rabbit`, `other` | Loài thú cưng. |
...
| `payment_method`| VARCHAR | Không | | Loại: `cod`, `vnpay`, `sepay` | Phương thức thanh toán. |
...
### 14. Bảng `payments`
Chi tiết lịch sử giao dịch thanh toán đơn hàng (VNPay/SePay).
...
| `method` | VARCHAR | Không | | Loại: `cod`, `vnpay`, `sepay` | Phương thức thanh toán. |
...
| `external_txn_id`| VARCHAR | Có | UK | | Mã giao dịch từ đối tác cổng (VNPay/Sepay). |
...
| `source_url` | VARCHAR | Có | | | URL nguồn bên ngoài tham chiếu. |
| `category` | VARCHAR | Không | | Phân loại: `nutrition`, `health`, `training`, `grooming`, `breed` | Chủ đề kiến thức. |
